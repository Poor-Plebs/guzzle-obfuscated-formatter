...
namespace PoorPlebs\GuzzleObfuscatedFormatter\GuzzleHttp;

use GuzzleHttp\MessageFormatterInterface;
use GuzzleHttp\Psr7\Utils;
use PoorPlebs\GuzzleObfuscatedFormatter\GuzzleHttp\Psr7\Message;
...
final class ObfuscatedMessageFormatter implements MessageFormatterInterface
{
     ...
     * @var string
     */
    public const string CLF = '{hostname} {req_header_User-Agent} - [{date_common_log}] "{method} {target} HTTP/{version}" {code} {res_header_Content-Length}';
    ...
    private function obfuscateRequestBody(RequestInterface $req): RequestInterface
    {
        ...
        $match = false;
        array_walk_recursive($reqJson, function (mixed &$value, int|string $key) use (&$match): void {
            if (!is_string($key)) {
                return;
            }
    ...
    private function obfuscateResponseBody(ResponseInterface $res): ResponseInterface
    {
        ...
        $match = false;
        array_walk_recursive($resJson, function (mixed &$value, int|string $key) use (&$match): void {
            if (!is_string($key)) {
                return;
            }
